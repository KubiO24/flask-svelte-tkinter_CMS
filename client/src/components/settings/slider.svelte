...
<div class='sliderContainer'>
    <div class="durationContainer">
        ...
        <input type="range" name="duration" id="sliderDuration" bind:value={sliderDuration} on:change={changeSliderDuration} min=10 max=1000>
    </div>
    
    {#each sliderList as slide}
        <SliderElement {slide} bind:selectedSetting={selectedSetting} sortSlider={() => sortSlider()} bind:sliderList={sliderList}/>
	{/each}
    {#if sliderListLoaded}
        <SliderElement bind:sliderList={sliderList} />
    {/if}
</div>
